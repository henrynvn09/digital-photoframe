...
configure_schedule() {
    ...
    print_info "  Weekends (Sat/Sun): ON at 8:00 AM, OFF at 8:45 PM"
    print_info "  Weekdays (Mon-Fri): ON at 4:00 PM, OFF at 8:45 PM"
    echo ""
    
    if ! ask_yes_no "Use default schedule?"; then
        print_info "Enter custom schedule times (24-hour format)"
        echo ""
        
        # Get weekend ON time
        read -rp "Weekend ON time (HH:MM, default 08:00): " weekend_on
        weekend_on="${weekend_on:-08:00}"
        
        # Get weekday ON time
        read -rp "Weekday ON time (HH:MM, default 16:00): " weekday_on
        weekday_on="${weekday_on:-16:00}"
        
        # Get daily OFF time
        read -rp "Daily OFF time (HH:MM, default 20:45): " off_time
        off_time="${off_time:-20:45}"
        
        # Parse times into hours and minutes
        WEEKEND_ON_HOUR="${weekend_on%%:*}"
        WEEKEND_ON_MIN="${weekend_on##*:}"
        WEEKDAY_ON_HOUR="${weekday_on%%:*}"
        WEEKDAY_ON_MIN="${weekday_on##*:}"
        OFF_HOUR="${off_time%%:*}"
        OFF_MIN="${off_time##*:}"
    else
        # Use defaults
...
# Weekend (Saturday, Sunday) ON time
WEEKEND_ON_HOUR=$WEEKEND_ON_HOUR
WEEKEND_ON_MIN=$WEEKEND_ON_MIN

# Weekday (Monday-Friday) ON time
WEEKDAY_ON_HOUR=$WEEKDAY_ON_HOUR
WEEKDAY_ON_MIN=$WEEKDAY_ON_MIN

# Daily OFF time (applies every day)
OFF_HOUR=$OFF_HOUR
OFF_MIN=$OFF_MIN
EOF
    
    print_success "Schedule configured:"
    print_info "  Weekends: ON at $(printf '%02d:%02d' $WEEKEND_ON_HOUR $WEEKEND_ON_MIN), OFF at $(printf '%02d:%02d' $OFF_HOUR $OFF_MIN)"
    print_info "  Weekdays: ON at $(printf '%02d:%02d' $WEEKDAY_ON_HOUR $WEEKDAY_ON_MIN), OFF at $(printf '%02d:%02d' $OFF_HOUR $OFF_MIN)"
    echo ""
}
...
print_summary() {
    ...
    if [[ "$INSTALLED_SYSTEMD" == true ]]; then
        ...
        print_info "  - Weekends: ON at $(printf '%02d:%02d' ${WEEKEND_ON_HOUR:-8} ${WEEKEND_ON_MIN:-0}), OFF at $(printf '%02d:%02d' ${OFF_HOUR:-20} ${OFF_MIN:-45})"
        print_info "  - Weekdays: ON at $(printf '%02d:%02d' ${WEEKDAY_ON_HOUR:-16} ${WEEKDAY_ON_MIN:-0}), OFF at $(printf '%02d:%02d' ${OFF_HOUR:-20} ${OFF_MIN:-45})"
    fi
